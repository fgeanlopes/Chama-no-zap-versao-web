...
export default function Home() { 
  const [values, setValues] = useState({viewDigit:'', sendDigit:''});
  ...
  const handleSetMessage  = (viewDigit) => viewDigit.length > 0 ? setMensagemBtn('Digite número completo') : setMensagemBtn('Digite o número acima') 

  const handleHabilitaBtn  = () => {
  ...
  const handleInputChange = useCallback((e)=>{
    ...
    handleSetMessage(viewDigit);

    setValues({...values, viewDigit});
  },[values.viewDigit]);

  useEffect(()=>{
    const viewDigit = values.viewDigit
    if(viewDigit.length === 15){
      const sendDigit = handleReplaceSendDigit(viewDigit);
      handleHabilitaBtn();

      setValues({...values, sendDigit});
    }else setHabilitaBtn(false);
  },[values.viewDigit]);
  ...
  return (
    <>
      ...
      <section className="home" id="home">
        <div className="content">
          ...
          <div className="input_number">
            <input type="tel" value={values?.viewDigit} maxLength="15" placeholder="Digite número aqui" onChange={(e)=>{handleInputChange(e)}}/>
            <svg xmlns="http://www.w3.org/2000/svg" className={`${values?.viewDigit.length == 15 ? 'check' : 'none'}`} viewBox="0 0 166 150.9"><path d="M0.3 96l62.4 54.1L165.6 0.3"/></svg>
          </div>
          
          {habilitaBtn ? 
            <a className="button_wp button btn_chamar bold" 
            target="_blank" href={`https://wa.me/55${values?.sendDigit}`}>
              <p>{mensagemBtn}</p>
      ...
      <section className="info">
        ...
        <a className="version" target="_blank" href={`https://web.whatsapp.com/send/55${values?.sendDigit}`}>V.4</a>
      </section>
    </>
  )
}